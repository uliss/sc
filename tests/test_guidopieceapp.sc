TestGuidoPieceApp : GuidoTest {
    ...
    test_widgets {
        ...
        var p = GuidoPieceApp.new("Partita", "J.S.Bach", "/partita");
        p.addWidget(\knob, NodeJS_Knob.new);
        this.expect(p.widget("knob")).to.be.not.nil_;
        p.showWidgets;
        this.expect(p).to.sendOSC_(
            "/guido/forward",
            "/guido/widget/add",
            "{\"max\": 1,\"size\": 100,\"min\": 0,\"parent\": \"ui-elements\",\"oscPath\": \"/ui\",\"value\": 0,\"idx\": \"knob7\",\"label\": \"\",\"type\": \"knob\"}");

        this.expect(p).listen.osc_("/guido/ui/knob7");

        p.hideWidgets;
        this.expect(p).listen.osc_("/guido/ui/knob7");
        this.expect(p).to.sendOSC_("/guido/forward", "/guido/widget/remove", "knob7");

        p.removeWidget("knob");
        this.expect(p).not.listen.osc_("/guido/ui/knob7");
        this.expect(p).to.sendOSC_("/guido/forward", "/guido/widget/remove", "knob7");
        this.expect(p.widget("knob")).to.be.nil_;
        p.free;
